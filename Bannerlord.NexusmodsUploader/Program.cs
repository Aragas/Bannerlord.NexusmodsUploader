...
namespace Bannerlord.NexusmodsUploader
{
    public static class Program
    {
        ...
        {
            var chromeOption = new ChromeOptions {BrowserVersion = "83.0.4103.3900"};
            chromeOption.AddArguments("start-maximized");
            using var driver = Environment.GetEnvironmentVariable("GITHUB_ACTIONS") != null
                ? new RemoteWebDriver(new Uri("http://localhost:4444/wd/hub"), chromeOption) {FileDetector = new LocalFileDetector()}
                : new ChromeDriver();
            {
